...
INSERT INTO metric_defs (code, label, unit, formula_note, display_order) VALUES
    ...
    ('debt_to_equity',    'Debt / Equity',         'ratio',   '(long_term_borrowings + short_term_borrowings) / total_equity, falling back to total_debt / total_equity when the source does not split borrowings.', 120),
    ('current_ratio',     'Current Ratio',         'ratio',   'current_assets / current_liabilities', 130),
    ('fcf',               'Free Cash Flow',        'currency','net_operating_cash_flow + capex. Capex is stored as printed in the cash flow statement, i.e. negative for an outflow, so this adds rather than subtracts.', 140),
    ('cash_conversion',   'Cash Conversion',       'ratio',   'nocfps / eps_basic. Below 1 over several years is a warning sign.', 150),
    ('payout_ratio',      'Payout Ratio',          'percent', 'dividend_per_share / eps_basic — the dividend DECLARED for the year. Differs from dividends-paid / net-profit, which lags a year.', 160),
